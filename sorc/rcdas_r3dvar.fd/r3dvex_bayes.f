...
!-------------- now coarse grid
  ...
  halo_widthc=nhaloc*dlatc
	...
	write(*,*) '>> mpp_compgrid_init from r3dbays'
  call mpp_compgrid_init(rxcglb,rycglb,halo_widthc,nxcglb,nycglb,nxc,nyc,2)

       write(*,*) '>>r3dvex_bayes associated 1 ', associated(gridcom%rxcom)
       allocate(rxc(nxc)) ; allocate(ryc(nyc))

       rxc=gridcom%rxcom
       ryc=gridcom%rycom
       write(*,*) '>>r3dvex_bayes associated 2 ', associated(gridcom%rxcom)
...
!    now define all filter constants 
  ...
  if(diagnostic) call outcovart(isofilter_t, &
	...
	write(*,*) '>> isoq=',isoq
  if(isoq) then
	write(*,*) ' >> goint get3berr with isofilter_q'
   ...
   deallocate(pg)
   deallocate(ksfc)
	write(*,*) ' >> goint get3berrq with isofilter_q'
