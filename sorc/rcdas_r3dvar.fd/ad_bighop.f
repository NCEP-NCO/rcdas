...
!     teta(ibight(m,i),1)=teta(ibight(m,i),1)+bight(m,i)*pbart(i)
     jj=ibight(m,i)-1
     ii=mod(jj,imeta*jmeta) + 1
     jj=(jj/(imeta*jmeta)) + 1
     teta(ii,jj)=teta(ii,jj)+bight(m,i)*pbart(i)
    end do
   end do
  end if
!---------------------------------------p
...
!---------------------------------------q
  if(mqdata.gt.0) then
   do i=1,mqdata
    do m=1,lbig3ges
!wne     qeta(ibighq(m,i),1)=qeta(ibighq(m,i),1)+bighq(m,i)*pbarq(i)
     jj=ibighq(m,i)-1
     ii=mod(jj,imeta*jmeta) + 1
     jj=(jj/(imeta*jmeta)) + 1
     qeta(ii,jj)=qeta(ii,jj)+bighq(m,i)*pbarq(i)
    end do
   end do
  end if
