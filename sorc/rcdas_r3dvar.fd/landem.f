...
!    call transmitance(eair, esnow, theta, theta_t, t12_v, t12_h)
     ...
     write(*,*) '>> theta=',theta
     write(*,*) '>> val=', theta,eair,esnow
     call subcsqrt(esnow,ctmp1)
     call subcsqrt(eair,ctmp2)
     write(*,*) '>> vala=', sin(theta),csqrt(eair),csqrt(esnow)
     write(*,*) '>> valb=', sin(theta),ctmp2,ctmp1
     write(*,*) '>> valc=', sin(theta),ctmp2*ctmp2,ctmp1*ctmp1
     write(*,*) '>> vald=', ctmp1, ctmp1*ctmp1
     write(*,*) '>> vale=', ctmp2, ctmp2*ctmp2

     write(*,*) '>> val2=',real(sin(theta)*csqrt(eair)/csqrt(esnow))
     write(*,*) '>> csqrt(1,1) = ',csqrt((1., 1.))
     theta_i = asin(real(sin(theta)*csqrt(eair)/csqrt(esnow)))
!     theta_i = asin(real(sin(theta)*ctmp2/ctmp1))
     ...
     write(*,*) '>> theta_t'
     call subcsqrt(esnow,ctmp1)
     call subcsqrt(esoil,ctmp2)
     theta_t = asin(real(sin(theta_i)*csqrt(esnow)/csqrt(esoil)))
!     theta_t = asin(real(sin(theta_i)*ctmp1/ctmp2))
...
!    t12_h    = 1.0
     call soil_diel(freq, t_soil, mv, rhob, rhos, sand, clay, esoil)
     call subcsqrt(eair,ctmp1)
     call subcsqrt(esoil,ctmp2)
     theta_t = asin(real(sin(theta)*csqrt(eair)/csqrt(esoil)))
!     theta_t = asin(real(sin(theta)*ctmp1/ctmp2))
...
!
	...
	write(*,*) '>>snow_diel in=',frequency,ep_real,ep_imag,rad,frac
...
!------------------------------------------
!
!  test .. own complex squrt
      subroutine subcsqrt(z,zsqrt)
! april 1977 version.  w. fullerton, c3, los alamos scientific lab.
      complex z, zsqrt

! wne      external cabs, sqrt
!
      x = real (z)
      y = aimag (z)
      r = cabs (z)
	write(*,*) 'subcsqrt=',z,x,y,r
!
      if (r.eq.0.) then
          zsqrt = (0.0, 0.0)
          return
      ...
      xtmp = sqrt ((r+abs(x))*0.5)
      ytmp = y*0.5/xtmp
	write(*,*) 'subcsqrt=',z,x,y,r,' x/ytmp=',xtmp,ytmp

      if (x.ge.0.) zsqrt = cmplx (xtmp, ytmp)
      if (y.eq.0.) y = 1.
      if (x.lt.0.) zsqrt = cmplx (abs(ytmp), sign(xtmp,y))
	write(*,*) 'subcsqrt zsqrt=',zsqrt
