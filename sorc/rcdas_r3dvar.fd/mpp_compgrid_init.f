...
!   first, distribute flags which tell if subdomain is on edge of global domain
        write(*,*) '>> mpp_compgrid_init args=',rxglb
        write(*,*) ' and more ryglb=',ryglb
        write(*,*) ' more args=',halo_width,nxglb,nyglb,nx,ny,igrid
...
!  store rxglb and ryglb in common
         ...
         if (.not.first) deallocate(gridcom%rxglbcom,stat=ier)
         allocate(gridcom%rxglbcom(nxglb))
         gridcom%nxglbcom=nxglb
         gridcom%rxglbcom=rxglb

         if (.not.first) deallocate(gridcom%ryglbcom,stat=ier)
         allocate(gridcom%ryglbcom(nyglb))
        write(*,*) '>> line 239 ',nyglb
         gridcom%nyglbcom=nyglb
        write(*,*) '>> line 240 ',ryglb
         gridcom%ryglbcom=ryglb
         
!  store rx and ry in common
        write(*,*) '>> deallocate %rxcom'
         if (.not.first) deallocate(gridcom%rxcom,stat=ier)
        write(*,*) '>> allocate %rxcom'
       write(*,*) '>>mpp_init 1associated ', associated(gridcom%rxcom)
         allocate(gridcom%rxcom(nx))
       write(*,*) '>>mpp_init 2associated ', associated(gridcom%rxcom)

        write(*,*) '>> nx=',nx
         gridcom%nxcom=nx
         gridcom%rxcom=rxglb(kx1_glb(mype,igrid):kx10_glb(mype,igrid))

         if (.not.first) deallocate(gridcom%rycom,stat=ier)
         allocate(gridcom%rycom(ny))
         gridcom%nycom=ny
         gridcom%rycom=ryglb(ky1_glb(mype,igrid):ky10_glb(mype,igrid))
