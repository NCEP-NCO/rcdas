...
!--------  Ht_psi, Hp_psi: psi is balanced variable. These define balanced part of T, pdr
  ...
  integer(4) iwgts_bightp(2,nxc*nyc)
        ...
        if(mype.eq.0) write(0,*)' gradj isofilter_q 0 ids,ide=', isofilter_q(1)%ids, isofilter_q(1)%ide
...
!   now apply H Eeta C
        ...
        if(mype.eq.0) write(0,*)' gradj isofilter_q 20 ids,ide=', isofilter_q(1)%ids, isofilter_q(1)%ide
...
! finally subtract from x
  ...
  if(userad) rpred=xpred-rpred
        if(mype.eq.0) write(0,*)' at gradj return'
