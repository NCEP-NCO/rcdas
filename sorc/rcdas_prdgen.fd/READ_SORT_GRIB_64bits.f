...
C 2013-01     EBISUZAKI   CHANGE JSTAT(N) for linux
C
...
C
         NFILE = TRIM(NFILE)
         IF (STAT(TRIM(NFILE),JSTAT).NE.0) THEN
           PRINT *,'ERROR IN FUNCTION STAT GETTING FILE STATS'
           IRET=99
           RETURN
         ELSE
C WNE 12-11           KBYTES = JSTAT(11)
           KBYTES = JSTAT(8)
...
C
         IF (IBUFSIZE.LT.KBYTES) THEN
           ...
           PRINT *,'THAN THE NUMBER OF BYTES IN THE FILE,' , TRIM(NFILE)
           PRINT *,'IBUFSIZE=',IBUFSIZE
           IRET = 40
           RETURN
         END IF
C
         OPEN (UNIT=LUGBIN,FILE=TRIM(NFILE),STATUS='OLD',
     &   ACCESS='DIRECT',
