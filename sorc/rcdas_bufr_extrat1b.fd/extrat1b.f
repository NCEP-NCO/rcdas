...
c                .      .    .                                       .
c main program: EXTRAT1B     time-windows, dupl-checks noaa-15 1b data
C   PRGMMR: KATZ             ORG: NP23        DATE: 1998-10-05
c
c abstract: reads in time-windowing specifications and list of noaa-15 
c   1b orbital files from standard input.  the 1b reports are then
...
c     unit 11  - binary noaa-15 1b orbital files
c
c   output files:
c     unit 06  - printout
c     unit 51  - binary noaa-15 1b dump file
c
...
c     Initialize variables.
      ...
      data lungrd        / 61 /
      data link / 198001*0 /
      data nhirs3, namsua, namsub / 20, 15, 5/
c
c***************************************************************************
c	Start extrtovs here
c
      CALL W3TAGB('EXTRAT1B',1998,0278,0047,'NP23   ')
c
c
c     Initialize variables.
      npass = 0
...
c     Get user input      
      read(stdin,input)  
      write(stdout,*)' '
      write(stdout,*)'Run extrat1b with variable settings below'
      write(stdout,input)
      write(stdout,*)
c
c
...
c        and obs skipping for every other good line.
         if (nchan.eq.nhirs3) then
            nspot = 56
            ichek = 14 + 8
            iskip = 2*iskipl
            iskip = max(2,iskip)
         elseif (nchan.eq.namsua) then
            nspot = 30
            ichek = 14 + 1
            iskip = 1
         elseif (nchan.eq.namsub) then
            nspot = 90
            ichek = 14 + 1
            iskip = 2*iskipl
            iskip = max(2,iskip)
         else
            write(stdout,*)'EXTRAT1B ERROR:  invalid number of channels'
            write(stdout,*)'   nchan = ',nchan
            call exit(12)
            CALL W3TAGE('EXTRAT1B')
         endif
c         
