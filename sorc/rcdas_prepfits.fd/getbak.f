...
C     
c     CALL adddate(igys,igms,igds,ighs,igf)
      call w3movdat(RICDAT,IFDATE,IVDATE)
c     vdate = (igys*1000000+igms*10000+igds*100+ighs)
...
c       if (iv.eq.5) jpds(5)=52  ! relative humidity
          ...
          IF (iret.eq.0) THEN
            ...
            DO kk = 1, itot
              ...
              ELSE IF (iv.eq.2) THEN
                t(ii,jj,l) = grid(kk)
c               if(grid(kk).lt.100.0) 
c    *           print*,'ii,jj,l,t(ii,jj,l)=',ii,jj,l,t(ii,jj,l)
                IF (mask(kk)) THEN
                  maskt(ii,jj,l) = 1
                END IF
