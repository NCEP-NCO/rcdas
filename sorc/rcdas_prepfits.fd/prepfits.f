...
C                .      .    .                                       .
C MAIN PROGRAM: PREPFITS     ADDS OBS/ANALYSIS INCREMENTS TO PREPBUFR FILE
C   PRGMMR: JANJIC           ORG: NP22        DATE: 99-01-20  
C
C ABSTRACT: PROGRAM ADDS OBS/ANALYSIS INCREMENTS TO PREPBUFR FILE
C
...
C     UNIT5      Records of model name  GRIB file / name  index file
C     UNIT11     LEVCAT namelist input file.  Example:
C                 &LEVCAT
C                 NUMLEV=39
C                 FIT=.T.,.T.,.F.,.F.,.T.,.T.,.T.,.F.,.F.,.F.
C                 &END
C     UNIT20     INPUT BUFR file created by editbufr
C     UNIT21     GRIB file for forecast
C     UNIT22     PREPFITS BUFR table file
C     UNIT23     GRIB index file for forecast
C
C   OUTPUT FILES:  NONE (SEE SUBROUTINE CHKOUT)
C       51      Output BUFR file with obs & forecast profiles
C
...
C 
      ...
      character*200 file(mxb), fndx(mxb)
      DIMENSION mate(mxr)
...
C     ------------------------------------------------------------
      ...
      PRINT '(" DATA VALID AT ",4I2.2)', iy, im, id, ih
...
C     --------------------------------------------------------
      ...
      DO n = 1, mxb+1
        READ (5,'(A8,1X,A200)',END=10) src(n), file(n)
        READ (5,'(A8,1X,A200)',END=10) src(n), fndx(n)
      END DO
...
C     --------------------------------------------------------------
      ...
      print*,'>> check backgrounds'
 
      print*,'before ibak loop'
      DO ibak = 1, nbak
        print*,'in ibak loop'
        CLOSE (luges)
        CLOSE (lugbi)
...
c       open(23,file=fndx(ibak),form='unformatted',iostat=ishi)
        print*,'before baopenr'
        print*,'file=',file(ibak)
        print*,'fndx=',fndx(ibak)
        call baopenr(21,trim(file(ibak)),ishl)
        call baopenr(23,trim(fndx(ibak)),ishi)
        print*,'ishl,ishi=',ishl,ishi
c       ishl=0
c       ishi=0
        print*,'after baopenr'
...
C     ----------------------------------
      print*,'>> read through prepbufr messages'
...
C       ----------------------
     ...
     +                  xyz(2,irep-1), xyz(1,irep+1), xyz(2,irep+1)
          ...
          hstid = hdr(1)
c         print*,'hstid=',hstid
c         print*,'staid,hdr(1)=',staid,hdr(1)
c         if(staid(1:5).eq.'71399') then
c         do i=1,nlev
c          print*,'obs(1,i),obs(3,i)=',obs(1,i),obs(3,i)
c         enddo
c         endif
...
C           ----------------------------------------------------------
            ...
            DO n = 1, nbak
              IF (n.lt.nbak.and.nofo(n).ne.1) THEN
                CALL raddate(vdate(n),abs(fhr(n)-fhr(n+1)),bdate)
